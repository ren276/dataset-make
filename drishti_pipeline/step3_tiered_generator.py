...
def assess_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # _tier_bucket drives which rows ENTER tiering (vs. being excluded as
    # "normal"). A row with fever_pattern=True but zero abnormal vitals
    # (_param_count==0) is a real, clinically common PHC scenario -- fever/
    # infection symptoms with otherwise-normal vitals -- and must not be
    # silently dropped before labeling, or the entire {"fever_pattern"}-only
    # ICD_MAPPING key (typhoid/dengue/UTI/gastroenteritis-alone) can never
    # realize (this was confirmed empirically: those codes had zero rows
    # until this fix). Such rows are folded into Tier 1 -- clinically the
    # mildest bucket, since there's no vital-sign derangement at all.
    df["_tier_bucket"] = df["_param_count"]
    fever_only = (df["_param_count"] == 0) & df["fever_pattern_flag"]
    df.loc[fever_only, "_tier_bucket"] = 1
...
def build_tiers(df: pd.DataFrame, seed: int, target_counts: dict = None) -> pd.DataFrame:
    """
    Partition df into Tiers 1-6 by exact _tier_bucket (== _param_count,
    except fever_pattern-only rows are folded into bucket 1 -- see
    assess_dataframe).
